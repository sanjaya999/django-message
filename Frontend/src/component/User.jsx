        ...
        response.forEach((convo) => {
          if (!sockets.current[convo.conversation_id]) {
            ...
            const socket = new WebSocket(
              `${wsScheme}://127.0.0.1:9000/ws/chat/${convo.conversation_id}`
            );
            sockets.current[convo.conversation_id] = socket;

            socket.onopen = () => {
              console.log(
                `WebSocket connected for conversation ${convo.conversation_id}`
              );
                ...
                if (!isViewingConversation) {
                  ...
                  setNotificationCounts((prevCounts) => ({
                    ...
                    [convo.conversation_id]:
                      (prevCounts[convo.conversation_id] || 0) + 1,
                  }));
                }
            ...
            socket.onclose = () => {
              console.log(
                `WebSocket closed for conversation ${convo.conversation_id}`
              );
      ...
      {conversations.length === 0 ? (
        ...
        conversations.map((convo) => (
          ...
          >
            ...
            <button onClick={() => startCall(convo.conversation_id)}>
              Start Call
            </button>
          </div>
        ))
